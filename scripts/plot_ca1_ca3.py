...
def my_sin(x, freq, amp, phase):
    return amp * np.sin(0.90 * ((2 * np.pi * freq * x / 1000) + phase))
...
#        ax6.plot(t, i_guess, color='green')
    ...
    plt.tight_layout()
    plt.savefig('ca1_ca3_ca3_theta_gamma-5secs.png', dpi=500)
...
#    xf = fftfreq(N, T)[:N//2]
#    plt.figure()
#    plt.plot(xf[:200], 1.0 / 10 * np.abs(yf[0:N//10]), label='Pyramidal cells')
#
#    yf = fft(B3)
#    xf = fftfreq(N, T)[:N//2]
#    plt.plot(xf[:200], 1.0 / 10 * np.abs(yf[0:N//10]), label='Basket cells')
#
#    yf = fft(BS3)
#    xf = fftfreq(N, T)[:N//2]
#    plt.plot(xf[:200], 1.0 / 10 * np.abs(yf[0:N//10]), label='Bistratified cells')
...
#        ax6.plot(t, s_guess, color='green')
    ...
    plt.tight_layout()
    plt.savefig('ca1_ca3_ca1_theta_gamma-5secs.png', dpi=500)
